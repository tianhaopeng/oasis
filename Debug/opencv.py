...
width, height, tempRgb = cap.read()[1].shape
zoom = 0
name = ''
user = ''
...
def getImg():
    ret,frame = cap.read()
    if zoom != 0:
        w = int(width*zoom/200)
        h = int(height*zoom/200)
        frame = frame[w:(width-w), h:(height-h)]
    ...
    while exitFlag == 0:
        ...
        while True:
            ...
            if data == b"" or data == b"q":
                ...
                if data[7] == b's':
                    zoom = int(data[6:7].decode('utf8'))
                else:
                    zoom = int(data[6:8].decode('utf8'))
        clientsocket.close()
    #quit()
